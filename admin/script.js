...
async function initDashboard() {
    ...
    try {
        ...
        console.log('🎮 Recharge gameIds (first 5):', rechargeValidator.recharges.slice(0, 5).map(r => r.gameId));

        // Step 4: Process and validate data
        showLoading(true, 'Processing and validating data...', 80);
...
function validateAndDisplayWinners() {
    ...
    // Filter: only entries with csvStatus = 'VALID' and copy to status field for validator
    const validEntries = allEntries
        .filter(e => e.csvStatus === 'VALID' || e.csvStatus === 'VÁLIDO')
        .map(e => ({ ...e, status: 'VALID' }));  // Copy csvStatus to status for validator
    console.log('🏆 Winner calc: Total entries:', allEntries.length, 'Valid entries:', validEntries.length);
    
    // Get winners from VALID entries only
...
function calculateWinnerPrizes(winners) {
    // Group by platform and matches (tier)
    const groups = {};
    winners.forEach(w => {
        const key = `${w.platform}_${w.validation.matches}`;
...
function applyWinnersFilters() {
    ...
    winnersState.filteredWinners = winnersState.allWinners.filter(win => { if(c && win.contest!==c) return false; if(d && win.drawDate!==normalizeDate(d)) return false; if(pt && win.validation.matches!==parseInt(pt,10)) return false; if(w && !win.whatsapp.toLowerCase().includes(w)) return false; return true; });
...
function setupWinnersListeners() {
    ...
    document.getElementById('exportWinnersBtn')?.addEventListener('click', () => {
        ...
        winnersState.filteredWinners.forEach(w => { const v=w.validation; csv+=`"${v.prizeTier.tier}","${v.matches}","${w.registrationDate} ${w.registrationTime}","${w.gameId}","${w.whatsapp}","${w.chosenNumbers.join(', ')}","${v.winningNumbers.join(', ')}","${v.matchedNumbers.join(', ')}","${w.drawDate}","${w.contest}","${w.ticketNumber}"\n`; });
